...
class DetalleVentas extends AbstractDBConnection implements Model, JsonSerializable
{
    ...
    function insert()
    {
        $query = "INSERT INTO parqueadero.detalle_ventas VALUES (:id,:venta_id,:producto_id,:cantidad,:precio_venta,:created_at)";
        if($this->save($query)){
    ...
    {
        $query = "UPDATE parqueadero.detalle_ventas SET 
    ...
    public static function searchForId($id) : ?DetalleVentas
    {
        try {
            if ($id > 0) {
                ...
                $getrow = $DetalleVenta->getRow("SELECT * FROM parqueadero.detalle_ventas WHERE id = ?", array($id));
    ...
    {
        return DetalleVentas::search("SELECT * FROM parqueadero.detalle_ventas");
    }
    ...
    {
        $result = DetalleVentas::search("SELECT id FROM parqueadero.detalle_ventas where venta_id = '" . $venta_id. "' and producto_id = '" . $producto_id. "'");
        if (count($result) > 0) {
            return true;
        } else {
            return false;
