...
class Fotos extends AbstractDBConnection implements Model, JsonSerializable
{
    ...
    {
        $query = "INSERT INTO parqueadero.fotos VALUES (:id, :nombre, :descripcion, :producto_id, :ruta, :estado, :created_at, :updated_at)";
    ...
    {
        $query = "UPDATE parqueadero.fotos SET 
    ...
    {
        return Fotos::search("SELECT * FROM parqueadero.fotos");
    }
    ...
    static function searchForId(int $id): ?object
    {
        try {
            if ($id > 0) {
                $Foto = new Fotos();
                $Foto->Connect();
                $getrow = $Foto->getRow("SELECT * FROM parqueadero.fotos WHERE id =?", array($id));
                $Foto->Disconnect();
