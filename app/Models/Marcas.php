...
class Marcas extends AbstractDBConnection implements Model, JsonSerializable
{
    ...
    {
        $this->productosMarca = Productos::search("SELECT * FROM parqueadero.productosYservicios WHERE marca_id = ".$this->id." and estado = 'Activo'");
    ...
    {
        $query = "INSERT INTO parqueadero.marcas VALUES (:id,:nombre,:descripcion,:estado,:created_at,:updated_at)";
    ...
    {
        $query = "UPDATE parqueadero.marcas SET 
    ...
    public static function searchForId($id) : ?Marcas
    {
        try {
            if ($id > 0) {
                $Marca = new Marcas();
                $Marca->Connect();
                $getrow = $Marca->getRow("SELECT * FROM parqueadero.marcas WHERE id =?", array($id));
                $Marca->Disconnect();
    ...
    {
        return Marcas::search("SELECT * FROM parqueadero.marcas");
    }
    ...
    public static function marcaRegistrada($nombre): bool
    {
        ...
        $result = Marcas::search("SELECT id FROM parqueadero.marcas where nombre = '" . $nombre. "'");
