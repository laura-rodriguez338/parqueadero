...
class Usuarios extends AbstractDBConnection implements Model, JsonSerializable
{
    ...
    {
        $query = "INSERT INTO parqueadero.usuarios VALUES (
    ...
    {
        $query = "UPDATE parqueadero.usuarios SET 
    ...
    public static function searchForId(int $id): ?Usuarios
    {
        try {
            if ($id > 0) {
                ...
                $tmpUsuario->Connect();
                $getrow = $tmpUsuario->getRow("SELECT * FROM parqueadero.usuarios WHERE id =?", array($id));
    ...
    {
        return Usuarios::search("SELECT * FROM parqueadero.usuarios");
    }
    ...
    {
        $result = Usuarios::search("SELECT * FROM parqueadero.usuarios where documento = " . $documento);
