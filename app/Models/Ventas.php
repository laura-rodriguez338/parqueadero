...
class Ventas extends AbstractDBConnection implements Model, JsonSerializable
{
    ...
    public function getDetalleVenta(): ?array
    {
        ...
        $this->detalleVenta = DetalleCompras::search('SELECT * FROM parqueadero.detalle_ventas where venta_id = '.$this->id);
    ...
    {
        $query = "INSERT INTO parqueadero.ventas VALUES (:id,:numero_serie,:cliente_id,:empleado_id,:fecha_venta,:monto,:estado,:created_at,:updated_at)";
    ...
    {
        $query = "UPDATE parqueadero.ventas SET 
    ...
    public static function searchForId($id) : ?Ventas
    {
        try {
            if ($id > 0) {
                $Venta = new Ventas();
                $Venta->Connect();
                $getrow = $Venta->getRow("SELECT * FROM parqueadero.ventas WHERE id =?", array($id));
                $Venta->Disconnect();
    ...
    {
        return Ventas::search("SELECT * FROM parqueadero.ventas");
    }
    ...
    public static function facturaRegistrada($numeroSerie): bool
    {
        ...
        $result = Compras::search("SELECT id FROM parqueadero.ventas where numero_serie = '" . $numeroSerie. "'");
